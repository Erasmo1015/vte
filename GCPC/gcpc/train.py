...
def make_wandb_name(cfg: DictConfig):
    if cfg.model.stage =='trl':
        name = f'stage1'
    elif cfg.model.stage =='pl':
        name = f'stage2'
    else:
        name = make_ckpt_filename(cfg)
    return name
...
def main(cfg: DictConfig):
    ...
    # goal_type = 'rtg' # HACK for gym only envs
    goal_dim = cfg.exp.goal_dim

    dm = D4RLDataModule(goal_type, cfg)
    if hasattr(cfg.model, 'stage'):
        dm.setup(stage=cfg.model.stage, custom=cfg.custom)
    else:
        dm.setup('fit', custom=cfg.custom)
    
    # print epoch number
    print(f'Epochs: {cfg.exp.epochs}')
    ...
    name = f'{cfg.env_name}-{make_wandb_name(cfg)}'
    if cfg.wandb.offline:
        wdb_logger = WandbLogger(name=name, save_dir=working_dir, version=None, offline=True, project=cfg.wandb.project, log_model=False)
    else:
        wdb_logger = WandbLogger(name=name, save_dir=working_dir, version=None, project=cfg.wandb.project, log_model=False)
