...
class MLPBNPolicy(nn.Module):
    ...
    def forward(self, observations: Tensor, goal: Tensor, obs_mask: Tensor):
        ...
        return pred_a
        # return pred_a, bottleneck
    
    def forward_full(self, observations: Tensor, full_obs_seq: Tensor, goal: Tensor, obs_mask: Tensor):
        batch_size, _, _ = observations.shape
        obs_mask_full = torch.ones(full_obs_seq.shape[0], full_obs_seq.shape[1], dtype=torch.bool)
        latent_future, _, _ = self.slotmae.encode(full_obs_seq, goal, obs_mask_full)
        o_keep = observations[obs_mask == 0].view(batch_size, -1, self.obs_dim)
        obs_goal = torch.cat([o_keep[:, -1:], goal], dim=-1)
...
class PolicyNet(PolicyLM):
    def __init__(self, seed: int, env_name: str, obs_dim: int, action_dim: int, goal_dim: int, lr: float, use_scheduler: bool, epochs: int, ctx_size: int, future_horizon: int, stage: str, num_eval_rollouts: int, eval_last_k: int, goal_type: str, model_config: DictConfig, **kwargs):
        ...
        # trajnet = TrajNet.load_from_checkpoint(checkpoint_path='/home/zichang/repo/GCPC/lightning_logs/pusher-medium-v1_GCPC/trajnet-seed:1-ctx:5-fh:20-lr:0.0001-bs:1024-epoch:60-d_emb:256-pdrop:0.1-n_enc:2-n_dec:1-n_hd:4-slot:4-mt:mae_rc/checkpoints/pusher-medium-v1-epoch:57-val_loss:0.002.ckpt', map_location=self.device)
    ...
    # The following functions encode full obs seq
    # eval funcs
    def init_eval_full(self, observations: Tensor, full_obs_seq: Tensor, goal: Tensor, obs_dim: int, action_dim: int, goal_dim: int):
        observations = full_obs_seq.unsqueeze(0, device=self.device) # 1 x ctx_size x obs_dim
        # goal: (goal_dim, )
        goal = goal.view(1, -1, goal_dim)
        actions = torch.zeros(size=(1, self.ctx_size, action_dim), device=self.device)  # 1 x ctx_size x action_dim
        return observations, actions, goal

    def ar_step_full(self, timestep: int, observations: Tensor, full_obs_seq: Tensor, actions: Tensor, goal: Tensor):
            obs_mask, _ = self.make_ar_mask(timestep)
            pred_a, bottleneck = self.model.forward_full(observations, full_obs_seq, goal, obs_mask)
            action = pred_a[0, 0]
            return action

    def ar_step_end_full(self, timestep: int, next_obs: Tensor, action: Tensor, obs_seq: Tensor, action_seq: Tensor):
        if timestep < self.ctx_size - 1:
            obs_seq[0, 1 + timestep] = next_obs
            new_obs_seq = obs_seq
            action_seq[0, timestep] = action
